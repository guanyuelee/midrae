...
def dataset_settings(dataset):
    if (dataset == 'mnist-full') | (dataset == 'mnist-test') | (dataset =='mnist-train'):
        kernel_sizes = [4, 5]
        strides = [2, 2]
        paddings = [0, 2]
        test_batch_size = 100
        ...
        paddings = [0, 2]
        test_batch_size = 100
    elif (dataset == 'coil20') | (dataset == 'coil100'):
        kernel_sizes = [4, 5]
        strides = [2, 2]
        paddings = [0, 2]
        test_batch_size = 100
...
def kmeans(encoder_val_clean, y, nClusters, y_pred_prev=None, weight_initilization='k-means++', seed=42, n_init=40,
    ...
    print('k-means: \t nmi =', normalized_mutual_info_score(y, y_pred), '\t arc =', adjusted_rand_score(y, y_pred),
          '\t {:s}acc = {:.4f} '.format(name, bestMap(y, y_pred)),
...
def iterate_minibatches(inputs, targets, batchsize, shuffle=False):
    ...
    for start_idx in range(0, len(inputs), batchsize):
        if shuffle:
            excerpt = indices[start_idx:min(start_idx + batchsize, len(inputs))]
        else:
...
def train_depict_ae(X, y, input_var, decoder, encoder, loss_recons, loss_recons_clean, num_clusters, output_path,
    ...
    if os.path.isfile(os.path.join(output_path, 'pretrained_params_best.pickle')) and continue_training:
        with open(os.path.join(output_path, 'pretrained_params_best.pickle'), "rb") as input_file:
            pretrained_params = pickle.load(input_file)
            lasagne.layers.set_all_param_values(decoder, pretrained_params)
    else:
        # TRAIN MODEL
        if verbose > 1:
        ...
        for epoch in range(num_epochs + 1):
            ...
            if validation_error < best_val:
                ...
                with open(os.path.join(output_path, 'pretrained_params.pickle'), "wb") as output_file:
            ...
            if (verbose > 1) & (epoch % 10 == 0):
                ...
                for batch in iterate_minibatches(X, y, test_batch_size, shuffle=False):
                    ...
                    if minibatch_flag:
                        ...
                        encoder_targets = np.concatenate((encoder_targets, targets), axis=0)

                kmeans(encoder_val_clean, encoder_targets, num_clusters, seed=seed, name='pretrain')

        last_params_values = lasagne.layers.get_all_param_values(decoder)
        with open(os.path.join(output_path, 'pretrained_params_best.pickle'), "wb") as output_file:
            pickle.dump(best_params_values, output_file)
        with open(os.path.join(output_path, 'pretrained_params.pickle'),
                  "wb") as output_file:
...
def clustering(dataset, X, y, input_var, encoder, num_clusters, output_path, test_batch_size=100, seed=42,
               continue_training=False, methods='ac-pic', skip=False):
    if not skip:
        encoder_clean = lasagne.layers.get_output(encoder, deterministic=True)
        encoder_clean_function = theano.function([input_var], encoder_clean)
        ...
        for batch in iterate_minibatches(X, y, test_batch_size, shuffle=False):
            ...
            if minibatch_flag:
                ...
                encoder_val_clean = np.concatenate((encoder_val_clean, minibatch_x), axis=0)
                encoder_y = np.concatenate((encoder_y, targets), axis=0)

        # Check kmeans results
        kmeans(encoder_val_clean, encoder_y, num_clusters, seed=seed, name='apply')
    encoder_val_clean = X
    y = y.argmax(1)
    initial_time = timeit.default_timer()
    if methods == 'kmeans':
        # K-means on MdA Features
        centroids, inertia, y_pred = kmeans(encoder_val_clean, encoder_y, num_clusters, seed=seed, name='apply')
        y_pred = (np.array(y_pred)).reshape(np.array(y_pred).shape[0], )
        y_pred = y_pred - 1
    elif methods == 'ac-pic':
        # AC-PIC on MdA Features
        if os.path.isfile(os.path.join(output_path, 'ac_pic_best.pickle')) & continue_training:
            with open(os.path.join(output_path, 'ac_pic_best.pickle'), "rb") as input_file:
            ...
            with open(os.path.join(output_path, 'ac_pic_best.pickle'), "wb") as output_file:
        ...
        print('AC-PIC: \t nmi =  ', normalized_mutual_info_score(y, y_pred),
              ...
              '\t apply_acc = {:.4f} '.format(bestMap(y, y_pred)),
...
def train_depict(dataset, X, y, input_var, decoder, encoder, loss_recons, num_clusters, y_pred, output_path,
    ...
    if init_flag:
        if os.path.isfile(os.path.join(output_path, 'initialize_weights1.pickle')) & continue_training:
            with open(os.path.join(output_path, 'initialize_weights1.pickle'), "rb") as input_file:
            ...
            for epoch in range(1000):
                ...
                print('epoch:', epoch + 1, '\t nmi = {:.4f}  '.format(normalized_mutual_info_score(y, y_pred)),
                      ...
                      '\t final_acc = {:.4f} '.format(bestMap(y, y_pred)),
            ...
            with open(os.path.join(output_path, 'initialize_weights.pickle'), "wb") as output_file:
    ...
    print('epoch: 0', '\t nmi = {:.4f}  '.format(normalized_mutual_info_score(y, y_pred)),
          '\t arc = {:.4f} '.format(adjusted_rand_score(y, y_pred)),
          '\t final_acc = {:.4f} '.format(bestMap(y, y_pred)))
    if os.path.isfile(os.path.join(output_path, 'final_weights.pickle')) & continue_training:
        with open(os.path.join(output_path, 'final_weights.pickle'),
                  "rb") as input_file:
            weights = pickle.load(input_file)
            lasagne.layers.set_all_param_values([decoder, network2], weights)
    else:
        for epoch in range(num_epochs):
            ...
            if mean_squared_error(y_prob, y_prob_prev) < 1e-7:
                with open(os.path.join(output_path, 'final_weights.pickle'), "wb") as output_file:
                    pickle.dump(lasagne.layers.get_all_param_values([decoder, network2]), output_file)
                break
            ...
            print('epoch:', epoch + 1, '\t nmi = {:.4f}  '.format(normalized_mutual_info_score(y, y_pred)),
                  '\t arc = {:.4f} '.format(adjusted_rand_score(y, y_pred)),
                  '\t final_acc = {:.4f} '.format(bestMap(y, y_pred)), '\t loss= {:.10f}'.format(train_err / num_batches),
