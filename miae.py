...
# !/usr/bin/env python
"""
MI-AE: Replace the linear interpolation in ACAi with multidimensional interpolation.
"""
...
from lib.mmd import mmd_penalty
import numpy as np
...
class MIAE(train.AE):
    ...
    def model(self, latent, depth, scales, advweight, advdepth, reg):
        ...
        # alpha is the same shape of encode.
        alpha = tf.random_uniform(tf.shape(encode), 0, 1)
        alpha = 0.5 - tf.abs(alpha - 0.5)  # Make interval [0, 0.5]
        encode_mix = alpha * encode + (1 - alpha) * encode[::-1]
        decode_mix = decoder(encode_mix)

        loss_disc = tf.reduce_mean(tf.square(disc(decode_mix) - alpha))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer(
        ...
        'latent_width ** 2.')
    flags.DEFINE_integer('latent_width', 1, 'Width of the latent space.')
