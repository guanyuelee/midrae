...
#!/usr/bin/env python
"""
Add dual regularization to ACAI while keep linear interpolation unchanged.
"""
...
from lib import data, layers, train, utils, classifiers, eval, mmd

FLAGS = flags.FLAGS


class DRACAI(train.AE):

    def model(self, latent, depth, scales, advweight, advdepth, reg, wgt_mmd, advnoise, advfake):
        ...
        loss_mmd = tf.nn.relu(mmd.mmd2(encode_flat, h_flat))
        ...
        alpha_noise = tf.random_uniform([tf.shape(encode)[0], 1, 1, 1], 0, 0.5)
        ...
        with tf.control_dependencies(update_ops):
            train_ae = tf.train.AdamOptimizer(FLAGS.lr).minimize(
                loss_ae + advweight * loss_ae_disc + wgt_mmd * loss_mmd +
                advnoise * loss_ae_disc_noise + advfake * loss_ae_disc_fake,
                var_list=ae_vars)
            train_d = tf.train.AdamOptimizer(FLAGS.lr).minimize(
                loss_disc + loss_disc_real + loss_disc_noise + loss_disc_fake,
                var_list=disc_vars)
...
if __name__ == '__main__':
    ...
    flags.DEFINE_float('wgt_mmd', 1.0, 'weight of mmd.')
    flags.DEFINE_float('advnoise', 0.1, 'weight of noise')
    flags.DEFINE_float('advfake', 0.1, 'weight of fake')
    app.run(main)
