...
from lib import data, utils, functions
import numpy as np
...
def cluster(train_latents, train_labels, test_latents, test_labels, method='kmeans'):
    if method=='kmeans':
        ...
        for i in range(n_try):
            train_ind = np.random.choice(train_latents.shape[0], N, replace=False).astype(np.int32)
            test_ind = np.random.choice(test_latents.shape[0], N, replace=False).astype(np.int32)
            x = train_latents[train_ind]
            x_l = train_labels[train_ind]
            x_test = test_latents[test_ind]
            x_test_l = test_labels[test_ind]
            x_pred, _ = functions.clustering(None, x, x_l, None, None, 10, './TRAIN', None, None, False, method, skip=True)
            x_test_pred, _ = functions.clustering(None, x_test, x_test_l, None, None, 10, './TRAIN', None, None, False,
                                                  method, skip=True)
    else:
